...
const promptUser = () => {
  const schema = {
    properties: {
      ...
      cassandra: {
        pattern: /^y(?:es)?|n(?:o)?$/i,
        description: 'Will you need Cassandra',
        message: 'Must be yes or no',
        default: 'no',
        ask() {
          return prompt.history('isService').value.toLowerCase().startsWith('y');
        },
      },
      cassandra_keyspace: {
        pattern: /^\S*$/,
        message: 'If you have more then one key space use CSV syntax',
        description: 'What key spaces will you need',
        default: path.basename(process.cwd()).replace(/-([a-z])/g, g => g[1].toUpperCase()),
        ask() {
          const cassandra = prompt.history('cassandra');
          return cassandra && cassandra.value.toLowerCase().startsWith('y');
        },
      },
      mongo: {
        pattern: /^y(?:es)?|n(?:o)?$/i,
        description: 'Will you need Mongo',
        message: 'Must be yes or no',
        default: 'no',
        ask() {
          const cassandra = prompt.history('cassandra');
          return prompt.history('isService').value.toLowerCase().startsWith('y')
            && cassandra && cassandra.value.toLowerCase().startsWith('n');
        },
      },
      passport: {
        pattern: /^y(?:es)?|n(?:o)?$/i,
        description: 'Will you need Passport',
        message: 'Must be yes or no',
        default: 'no',
        ask() {
          return prompt.history('isService').value.toLowerCase().startsWith('y');
      ...
      frontend: {
        pattern: /^y(?:es)?|n(?:o)?$/i,
        description: 'Will you need a frontend',
        message: 'Must be yes or no',
        default: 'no',
        ask() {
          return prompt.history('isService').value.toLowerCase().startsWith('y');
