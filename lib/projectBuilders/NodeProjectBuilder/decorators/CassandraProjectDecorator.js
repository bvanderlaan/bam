...
function addUsers() {
  this.files.src.users = {
    ...
    'user.model.js': stripIndent`
      ...
      const schema = new cassie.Schema({
        id: {
          type: cassie.types.String,
          required: true,
        },
        display_name: {
          type: cassie.types.String,
      ...
      schema.validatePassword = function(password) {
        return bcrypt.compareSync(password, this.local_password);
      }

      module.exports = {
        createModel() {
          return cassie.model('User', schema);
        },

        getModel() {
          return cassie.model('User');
        },
      };`,
  };
}
