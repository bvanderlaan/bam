...
function updateDocker() {
  ...
  devCompose.services[this.name].environment.push(`APP_PUBLIC_PATH=http://localhost:4000/${this.name}`);
...
function createEntryPoint() {
  const contents = {
    config: {
      ...
      // TODO: Update to swagger v3
      const { host, path: basePath } = parse(nconf.get('app_public_path') || '');
      ...
      module.exports = {
        ...
        spec() {
          ...
          return Object.assign(swaggerJSDoc(options), securitySpec);
        },
      };`,
    },
    setup: {
...
#### Install Node.js
The version of Node.js which ${this.name} supports is \`v6.x\` and has been tested against \`v6.9.1\`.
See [How to install Node.js v6.x on Ubuntu 14 (Trusty)](https://www.digitalocean.com/community/tutorials/how-to-install-node-js-on-an-ubuntu-14-04-server#how-to-install-using-a-ppa) and use the \`https://deb.nodesource.com/setup_6.x\` script to install Node.js.
...
The above will install the latest version of the \`v6.x\` branch of node. To get more control over the minor version (lets say you want to install \`v6.9.1\` instead of the latest) you could look at using [NVM](https://www.digitalocean.com/community/tutorials/how-to-install-node-js-on-an-ubuntu-14-04-server#how-to-install-using-nvm) or some other Node installation mechanism.
